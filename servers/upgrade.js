...
export async function main(ns) {
  ...
  while (true) {
    ...
    var server = servers.reduce((prev, current) => (prev.ram() < current.ram()) ? prev : current)
    var index = servers.findIndex(x => x.name == server.name);
    var newRam = server.ram() * 2;
    if (maxRam < newRam) {
      break;
    }
    ...
    var threads = Math.floor(newRam / 3.5 / targets.length);
    while (threads == 0) {
